...
/*   Updated: 2023/02/28 12:03:58 by obibby           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parking_light_side_right(t_car *car)
{
	...
	if (!EasterEggLightsEE.ParkingLightRight)
	{
		...
		alpha = 255 - EasterEggLightsEE.ParkingLightRightPWM * 255 * 0.0009;
	while (++x < 485)
	{
		y = y_base - y_length;
...
void	parking_light_side_left(t_car *car)
{
	...
	if (!EasterEggLightsEE.ParkingLightLeft)
	{
		...
		alpha = 255 - EasterEggLightsEE.ParkingLightLeftPWM * 255 * 0.0009;
	while (++x < 128)
	{
		y = y_base - y_length;
		while (++y <  y_base + y_length)
